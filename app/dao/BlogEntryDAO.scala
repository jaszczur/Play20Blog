  ...
  override def find(id: Long): Option[BlogEntry] = {
    DB.withConnection { implicit connection =>
      SQL("select * from blog_entry join location on location.id = blog_entry.location_id where blog_entry.id = {id}").on(
        'id -> id).as(fullParser *).headOption
    }
  }
    ...
    DB.withConnection { implicit connection =>
      ...
      SQL(
        ...
        """).on(
          ...
          'name -> entry.location.name).executeUpdate()
      ...
      SQL(
        ...
        """).on(
          ...
          'locationId -> id).executeUpdate()

      entry.copy(id = Id(id))
    }
  }

  private lazy val h2DateFormatter = new java.text.SimpleDateFormat("yyyy-MM-dd HH:mm:ss")
  private def formatDate(date: Date): String = {
    h2DateFormatter.format(date)
  }
}
